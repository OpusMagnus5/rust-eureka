...
*/
use warehouse::{CardioExercise, GymWorkout, WeightliftingExercise, CardioTool};

fn main() {
    let cardio: CardioExercise = CardioExercise::new("Monday".to_string(), CardioTool::Bike, 15);
    let weightlifting = WeightliftingExercise::new("Plank".to_string(), 5);
    let gym_workout = GymWorkout::new(cardio, weightlifting);
